...
function Panel({ f, i }) {
  const { bodyRef, bind } = useTilt(5)
  return (
    <div {...bind}>
      <div ref={bodyRef} className="tilt-body">
        ...
        <Link to={`/collection?family=${f.id}`} className="group relative block overflow-hidden border hairline transition-colors duration-700 hover:border-gold/60">
          ...
          <div className="absolute inset-x-0 bottom-0 p-7 md:p-9">
            ...
            <span className="mt-6 inline-flex items-center gap-3 text-[10.5px] uppercase tracking-[0.3em] gold-text">
              Discover <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border-metal text-gold transition-transform duration-700 group-hover:translate-x-2"><ArrowRight size={13} strokeWidth={1.25} /></span>
            </span>
          </div>
        </Link>
      </div>
