...
export default function Bespoke() {
  ...
  return (
    <>
      ...
      <section data-tone="dark" className="bg-black text-ivory py-24 md:py-36">
        <div className="container-site grid gap-16 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            ...
            <p className="mt-3 text-[11px] uppercase tracking-[0.24em] text-ivory/50">A finished silk piece, presented in the hall</p>
          </Reveal>
            ...
            {STEPS.map((s, i) => (
              <Reveal as="li" key={s.n} delay={i * 90} className="grid grid-cols-[56px_1fr] gap-6 border-t hairline pt-8">
                ...
                <div>
                  ...
                  <p className="mt-3 text-[15px] leading-relaxed text-ivory/60">{s.text}</p>
                </div>
              </Reveal>
            ))}
          </ol>
      ...
      <section id="home-experience" data-tone="dark" className="bg-walnut text-ivory py-24 md:py-36 scroll-mt-[var(--header-h)]">
        <div className="container-site grid gap-14 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            <p className="eyebrow">The private home experience</p>
          ...
          <div className="lg:col-span-6 lg:col-start-7">
            <ol className="space-y-10">
              {HOME_STEPS.map(([t, d], i) => (
                <Reveal as="li" key={t} delay={i * 90} className="grid grid-cols-[56px_1fr] gap-6 border-t border-ivory/20 pt-8">
                  <span className="serif text-[28px] text-gold">0{i + 1}</span>
                  <div><h3 className="text-[clamp(24px,2.4vw,30px)]">{t}</h3><p className="mt-3 text-[15px] leading-relaxed text-ivory/60">{d}</p></div>
                </Reveal>
              ))}
            </ol>
            <Reveal delay={200} className="mt-12 flex flex-col gap-4 sm:flex-row">
              ...
              <Link to="/collection" className="btn btn-ivory">Choose a Piece</Link>
            </Reveal>
            <p className="mt-6 text-[12px] text-ivory/55">Anywhere in Europe. No payment is taken and nothing is committed; every visit is confirmed personally within one working day.</p>
          </div>
        </div>
      </section>

      <section id="consultation" data-tone="dark" className="bg-walnut text-ivory py-24 md:py-36 scroll-mt-[var(--header-h)]">
        <div className="container-site grid gap-14 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            <p className="eyebrow">Start a Project</p>
            <h2 className="mt-5 text-[clamp(36px,4.6vw,60px)]">Book a Private Consultation</h2>
            <p className="mt-6 text-[16px] leading-relaxed text-ivory/60">Tell us a little about the room and we will arrange a conversation at the atelier, at your home, or by video. We reply personally within two working days.</p>
          </Reveal>
          <Reveal delay={120} className="lg:col-span-6 lg:col-start-7">
            {done ? (
              ...
              <form onSubmit={submit} noValidate className="grid gap-7 sm:grid-cols-2">
                <div>
                  <label htmlFor="b-name" className="label text-ivory/60">Name</label>
                  <input id="b-name" className="field" value={form.name} onChange={set('name')} autoComplete="name" aria-invalid={Boolean(errors.name)} />
                  {errors.name && <p className="error-text text-[#d9a49a]">{errors.name}</p>}
                </div>
                <div>
                  <label htmlFor="b-email" className="label text-ivory/60">Email</label>
                  <input id="b-email" type="email" className="field" value={form.email} onChange={set('email')} autoComplete="email" aria-invalid={Boolean(errors.email)} />
                  {errors.email && <p className="error-text text-[#d9a49a]">{errors.email}</p>}
                </div>
                <div>
                  <label htmlFor="b-room" className="label text-ivory/60">Room</label>
                  <input id="b-room" className="field" value={form.room} onChange={set('room')} placeholder="Living room, gallery, bedroom…" aria-invalid={Boolean(errors.room)} />
                  {errors.room && <p className="error-text text-[#d9a49a]">{errors.room}</p>}
                </div>
                <div>
                  <label htmlFor="b-size" className="label text-ivory/60">Approximate size</label>
                  <input id="b-size" className="field" value={form.size} onChange={set('size')} placeholder="e.g. 300 × 400 cm" />
                </div>
                <div className="sm:col-span-2">
                  <label htmlFor="b-budget" className="label text-ivory/60">Budget</label>
                  <select id="b-budget" className="field" value={form.budget} onChange={set('budget')}>
                    <option value="">Select a range</option>
                    {BUDGETS.map((b) => <option key={b}>{b}</option>)}
                  </select>
                </div>
                <div className="sm:col-span-2">
                  <label htmlFor="b-notes" className="label text-ivory/60">Notes</label>
                  <textarea id="b-notes" rows={4} className="field resize-none" value={form.notes} onChange={set('notes')} placeholder="Palette, motifs you love, anything at all." />
                </div>
                <div className="sm:col-span-2">
                  <button type="submit" className="btn btn-solid">Request a Consultation</button>
                </div>
              </form>
            )}
          </Reveal>
