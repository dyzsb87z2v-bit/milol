...
export default function About() {
  useSeo({ title: 'About', description: 'MILAEDIA is a Berlin-based gallery for exceptional handmade Persian and Turkish carpets, serving collectors, designers and homeowners across Europe.' })
  return (
    <>
      <PageHero eyebrow="About · Berlin" title="Persian Heritage. European Elegance." intro="MILAEDIA is a Berlin-based gallery serving collectors, designers and homeowners across Europe. The collection is kept deliberately small: a rug is not chosen to fill a catalogue, it is chosen because it is worth keeping." image={{ src: '/img/atelier-salon.webp', small: '/img/atelier-salon-sm.webp' }} alt="A Berlin salon at dusk with a large medallion carpet beneath crystal chandeliers" />
      ...
      <section data-tone="dark" className="bg-black text-ivory pb-24 md:pb-36">
        <div className="container-site">
          ...
          <Reveal className="mx-auto mt-20 max-w-3xl text-center">
            ...
            <p className="mt-5 eyebrow text-ivory/55">The MILAEDIA atelier</p>
          </Reveal>
        </div>
      </section>
      ...
      <section data-tone="dark" className="bg-walnut text-ivory py-24 md:py-36">
        <div className="container-site">
          <Reveal className="max-w-2xl">
            <p className="eyebrow">Four commitments</p>
          ...
          <dl className="mt-16 grid gap-10 md:grid-cols-2 lg:grid-cols-4">
            {VALUES.map(([t, d], i) => (
              <Reveal key={t} delay={i * 90} className="border-t hairline pt-6">
                <dt className="text-[26px]">{t}</dt>
                <dd className="mt-3 text-[15px] leading-relaxed text-ivory/60">{d}</dd>
              </Reveal>
            ))}
          </dl>
        </div>
      ...
      <section id="how-we-choose" data-tone="dark" className="bg-black text-ivory py-24 md:py-36 scroll-mt-[var(--header-h)]">
        <div className="container-site grid gap-14 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            <p className="eyebrow">How pieces are chosen</p>
            ...
            <div className="mt-10 grid gap-10 sm:grid-cols-2 lg:grid-cols-1">
              <div>
                <p className="eyebrow text-ivory/55">Examined before acquisition</p>
                <ul className="mt-4 space-y-2 text-[14px] text-ivory/80">{EXAMINED.map((x) => <li key={x} className="flex gap-3"><span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gold" />{x}</li>)}</ul>
              </div>
              <div>
                <p className="eyebrow text-ivory/55">Grounds for rejection</p>
                <ul className="mt-4 space-y-2 text-[14px] text-ivory/80">{REJECTED.map((x) => <li key={x} className="flex gap-3"><span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gold" />{x}</li>)}</ul>
              </div>
            </div>
          </Reveal>
          <Reveal delay={120} className="lg:col-span-6 lg:col-start-7 prose-luxe">
      ...
      <section data-tone="dark" className="bg-black text-ivory pb-24 md:pb-36">
        <div className="container-site">
          ...
          <Reveal className="max-w-2xl">
            ...
            <p className="mt-5 text-[16px] leading-relaxed text-ivory/60">Most of our work happens before a rug is ever listed, and much of it continues long after it has been placed.</p>
          </Reveal>
      ...
      <section id="authenticity" data-tone="dark" className="bg-walnut text-ivory py-24 md:py-36 scroll-mt-[var(--header-h)]">
        <div className="container-site grid gap-14 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
