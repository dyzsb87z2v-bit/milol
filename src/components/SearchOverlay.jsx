...
export default function SearchOverlay() {
  ...
  return (
    <Modal open={ui.searchOpen} onClose={closeSearch} label="Search" wrapperClassName="fixed inset-x-0 top-0 z-[90]" panelClassName="bg-ivory text-charcoal shadow-2xl max-h-[92vh] overflow-auto animate-[rise_0.6s_var(--ease)]">
      <div className="container-site py-6 md:py-10">
        <div className="flex items-center gap-4 border-b border-charcoal/30 pb-3">
          <Search size={20} strokeWidth={1.25} className="shrink-0 text-charcoal/60" aria-hidden="true" />
          ...
          <input
            ...
            className="serif w-full bg-transparent text-[clamp(26px,4vw,44px)] placeholder:text-charcoal/30 focus:outline-none"
          />
        ...
        {q.trim() === '' ? (
          <div className="py-8">
            <p className="eyebrow text-charcoal/50">Suggestions</p>
            <ul className="mt-4 flex flex-wrap gap-3">
              {SUGGESTIONS.map((s) => (
                <li key={s}><button type="button" onClick={() => setQ(s)} className="border hairline px-4 py-2 text-[12px] uppercase tracking-[0.2em] hover:border-charcoal transition-colors">{s}</button></li>
              ))}
            </ul>
          </div>
        ) : results.length === 0 ? (
          <p className="py-10 text-charcoal/60">No pieces match “{q}”. Try a colour, an origin or a weaving style.</p>
        ) : (
          <>
            <ul id="search-results" role="listbox" className="grid gap-x-8 py-6 md:grid-cols-2">
              {results.map((p, i) => (
                <li key={p.id} id={`sr-${p.id}`} role="option" aria-selected={active === i}>
                  <Link to={`/collection/${p.slug}`} onClick={closeSearch} onMouseEnter={() => setActive(i)} className={`flex items-center gap-5 border-b hairline py-4 transition-colors ${active === i ? 'bg-charcoal/[0.04]' : ''}`}>
                    <Img image={p.images[0]} alt="" className="h-20 w-16 shrink-0" sizes="64px" />
                    <div className="flex-1">
                      <p className="text-[20px] leading-tight">{p.name}</p>
                      <p className="mt-1 text-[11px] uppercase tracking-[0.2em] text-charcoal/55">{p.collection}</p>
                    </div>
