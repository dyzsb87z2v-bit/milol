      ...
      <section data-tone="light" className="bg-ivory text-charcoal pb-24 md:pb-36">
        <div className="container-site">
          <nav aria-label="Subjects" className="flex flex-wrap gap-x-8 gap-y-3 border-y hairline py-4">
            {CATEGORIES.map((c) => (
              <button key={c} type="button" onClick={() => setParams(c === 'All' ? {} : { c }, { replace: true })} aria-current={cat === c ? 'page' : undefined} className={`link-line ${cat === c ? '' : 'text-charcoal/50 after:scale-x-0 hover:text-charcoal hover:after:scale-x-100 hover:after:origin-left'}`}>{c}</button>
            ))}
          </nav>
          <p className="mt-6 text-[12px] text-charcoal/55" aria-live="polite">{items.length} {items.length === 1 ? 'article' : 'articles'}</p>
